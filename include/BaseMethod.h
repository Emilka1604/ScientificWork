...
public:
    ...
    std::vector<double> get_local_err() {
        return m_localError;
    }
...
protected:
    ...
    double localError(const std::vector<double>& res, const std::vector<double>& halfStepRes) {
        std::vector<double> discrepancy(res.size());
        for(std::size_t i = 0; i < res.size(); ++i) {
            discrepancy[i] = std::abs(res[i] - halfStepRes[i]);
        }
        auto norm = *std::max_element(discrepancy.begin(), discrepancy.end());
        return norm / (std::pow(2, p) - 1);
    }

    

    std::vector<double> step_vec;
    std::vector<double> m_localError;
