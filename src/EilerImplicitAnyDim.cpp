...
int main(int argc, char *argv[])
{
    ...
    QVector<double> h_vec, t, x1, x2, err;
    ...
    MainWindow w;
    w.CreateTable(h_vec, t, x1, x2, QVector<double>(t.size(), 0.0), 
                    QVector<std::size_t>(t.size(), 0), QVector<std::size_t>(t.size(), 0), err);
    w.setMaxLocalErr(*std::max_element(err.begin(), err.end()));
    w.Maketx1(t, x1);
    w.Maketx2(t, x2);
    w.Makex1x2(x1, x2);
    w.show();
